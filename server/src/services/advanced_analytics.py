"""Grades analytics service for comparative analysis and risk segmentation."""
...
class AdvancedAnalytics:
    """Analytics engine for grades comparison and risk analysis."""
    ...
    def get_period_comparison(
        ...
        Args:
            ...
            comparison_type: "class", "subject_teacher", or "subject"
            grade_level: Optional grade level filter
            class_id: Optional class filter

        Returns:
            Dict with comparison data for multi-series charts
        """
    ...
    def get_red_student_segmentation(
        ...
        # Segmentation by subject (students failing in each subject)
        by_subject: dict = {}
        for g in grades:
            if g.student_tz in red_student_tzs:
        ...
        by_subject_result = [
            {
                ...
                "average_grade": round(sum(data["grades"]) / len(data["grades"]), 2)
                if data["grades"]
                else 0,
            }
    ...
    def get_versus_comparison(
        ...
        Args:
            comparison_type: Type of entities to compare ("class", "teacher", "layer")
            entity_ids: List of entity IDs to compare
            period: Optional period filter
            metric: Metric to compare ("average_grade", "at_risk_count")

        Returns:
            Dict with series data for multi-entity chart
        """
        series = []
    ...
    def get_cascading_filter_options(
        ...
        Returns:
            Available options for classes, teachers, subjects
        """
        ...
        return {
            "classes": sorted(class_options, key=lambda x: x["class_name"]),
            "teachers": sorted(teacher_options, key=lambda x: x["name"]),
