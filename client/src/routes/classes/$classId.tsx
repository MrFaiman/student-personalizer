...
function ClassDetailPage() {
  ...
  const excellentGradeThreshold = useConfigStore(
    (s) => s.excellentGradeThreshold,
  );
  ...
  const { data: heatmapData, isLoading: isLoadingHeatmap } = useQuery({
    ...
    queryFn: () =>
      analyticsApi.getClassHeatmap(classId, {
        year: filters.year,
  ...
  const { data: rankings, isLoading: isLoadingRankings } = useQuery({
    ...
    queryFn: () =>
      analyticsApi.getClassRankings(classId, {
        year: filters.year,
        period: filters.period,
      }),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <PageHeader
        ...
        subtitle={t("detail.studentsAndLevel", {
          count: classInfo?.student_count || 0,
          level: classInfo?.grade_level || "",
        })}
        stats={[
          {
            value: classInfo?.average_grade?.toFixed(1) || "-",
            label: t("detail.average"),
          },
          {
            value: classInfo?.at_risk_count || 0,
            label: t("detail.atRisk"),
          ...
          <CardContent className="p-6">
            ...
            {isLoadingRankings ? (
              ...
              <Table>
                ...
                <TableBody>
                  {rankings.top.map(
                    (student: StudentRanking, index: number) => (
                      <TableRow key={student.student_tz}>
                        <TableCell>
