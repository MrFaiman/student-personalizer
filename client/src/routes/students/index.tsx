...
function StudentsListPage() {
    ...
    const debouncedSearch = useDebouncedValue(form.state.values.search, DEBOUNCE_DELAY_MS);
    ...
    const { data: dashboardStats } = useQuery({
        queryKey: ["dashboard-stats", filters.year, filters.period, selectedClassId],
        queryFn: () =>
    ...
    const activeYear = filters.year || (metadata?.years.length ? metadata.years[0] : undefined);
    ...
    const prevYearStr = activeYearIndex >= 0 && activeYearIndex < sortedYears.length - 1 ? sortedYears[activeYearIndex + 1] : undefined;
    ...
    const { data: students, isLoading } = useQuery({
        queryKey: ["students", filters.year, filters.period, selectedClassId, debouncedSearch, showAtRiskOnly, page, sort.column, sort.direction],
        queryFn: () =>
            studentsApi.list({
                year: filters.year,
                period: filters.period,
    ...
    const studentTrend = calculateTrend(dashboardStats?.total_students ?? students?.total ?? 0, prevDashboardStats?.total_students);
    const atRiskTrend = calculateTrend(dashboardStats?.at_risk_count ?? 0, prevDashboardStats?.at_risk_count);
        ...
        <div className="space-y-4">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t("list.subtitle")} {activeYear ? `(${formatHebrewYear(activeYear)})` : ""}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <StatCard
                    ...
                    trend={studentTrend !== undefined ? { value: studentTrend, label: tc("general.vsLastYear", "vs last year") } : undefined}
                />
                <StatCard
                    icon={AlertTriangle}
                    ...
                    trend={atRiskTrend !== undefined ? { value: atRiskTrend, label: tc("general.vsLastYear", "vs last year"), invertColors: true } : undefined}
                />
                <StatCard
                    icon={Users}
                ...
                <CardContent className="p-4">
                    <div className="flex flex-wrap gap-4 items-center">
                            ...
                            {(field) => (
                                <div className="relative flex-1 min-w-[200px]">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => { field.handleChange(e.target.value); resetPage(); }}
                                    />
                                </div>
                            )}
                        </form.Field>
                        <form.Field name="selectedClassId">
                            {(field) => (
                                <Select value={field.state.value} onValueChange={(v) => { field.handleChange(v as typeof field.state.value); resetPage(); }}>
                        ...
                        <form.Field name="showAtRiskOnly">
                            {(field) => (
                                <Button
                                    ...
                                    onClick={() => { field.handleChange(!field.state.value); resetPage(); }}
                                >
                ...
                <Table>
                    <TableHeader>
                        <TableRow className="bg-accent/50">
                            ...
                            <SortableTableHead column="student_name" sort={sort} onSort={toggleSort}>{tc("table.studentName")}</SortableTableHead>
                            <TableHead className="text-right font-bold">{tc("table.idNumber")}</TableHead>
                            <SortableTableHead column="class_name" sort={sort} onSort={toggleSort}>{tc("table.class")}</SortableTableHead>
                            <SortableTableHead column="average_grade" sort={sort} onSort={toggleSort}>{tc("table.averageGrade")}</SortableTableHead>
                            <SortableTableHead column="total_absences" sort={sort} onSort={toggleSort}>{tc("table.absences")}</SortableTableHead>
